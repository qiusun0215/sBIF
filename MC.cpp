                 ...
                 vectord2d &inter, vectord2d &weights)
{
    ...
    for (unsigned i=head; i<tail; ++i)
    {
        ...
        if (i==node_idx-1) continue;
        //p+=log(poisson_pmf(select_ligc[i],inter[node_idx][i]))*pow(weights[node_idx][i],2)*abs(node_idx-i);
        //p+=pow(select_ligc[i]-inter[node_idx][i],2);
        p+=log(poisson_pmf(select_ligc[i],inter[node_idx][i]))*pow(weights[node_idx][i],3);
    }
    return p;
}
                     ...
                     vectord2d &weights)
{
    ...
    for (unsigned i=head; i<tail; ++i)
    {
        ...
        p+=log(poisson_pmf(new_num,inter[node_idx][i]))*pow(weights[node_idx][i],3);
...
my_ensemble MC(vectord2d &inter, vectord2d &weights, unsigned n_samples, unsigned n_sphere,
